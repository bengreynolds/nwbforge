...
8. Run preview/build, inspect warnings or blocking issues, then execute NWB conversion when the session is ready.
...
- Direct-ingest group identity now survives into created conversion sessions through persisted group kind, pathway, canonical bundle context, grouping reason, and member/source summaries on each created source
