...
- Temporary `scripts/run_app.py` launcher for manual desktop testing during development
...
The temporary launcher is a development aid only. It uses demo conversion/package behavior so the current desktop shell can be exercised without changing the long-term packaging plan.
...
4. Expand the concrete PySide6 widget baseline with richer conversion/review detail, broader settings coverage, and app-level log configuration.
