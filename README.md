...
The current application state is still early and backend-heavy, but it is no longer UI-free. The supported route set now includes real NeuroConv-backed CSV, Excel, image, audio, FicTrac, and DeepLabCut conversions; a shared NeuroConv adapter framework exists for additional single-interface routes; supported-route packaging now uses category-first modules for both `supported/behavior/` and `supported/tabular/`; a dedicated workflow base exists for future combined NeuroConv pipelines; runtime contracts now exist for background execution, structured logging, and stage/progress/error reporting; developer setup now has a route-based package planning layer with `minimal`, `selected`, and `full` install modes; future setup/package-install UI work has backend `PackageManagementService`, `PackageInstallationService`, `ThreadedPackageInstallationExecutor`, and `PackageManagementController` boundaries to call; the repo includes toolkit-agnostic UI models under `src/nwbforge/ui/`; and it now also includes a concrete PySide6 widget baseline under `src/nwbforge/ui/qt/` for the shell, settings dialog, package-install dialog, and a sectioned conversion-session surface with dedicated summary, execution, validation/review, and artifact panes, plus run-overview metrics for stage/output/issues/artifacts, review-guidance summaries, validation/review controls, generated-artifact visibility, direct artifact-open actions, dedicated validation/review report shortcuts, a real `Choose Output...` save dialog, docked log viewer, shell-level warning dialogs, opt-in composite file-plus-viewer logging, and a tabbed workspace for run overview, review work, and artifacts. A real desktop bootstrap/composition module now exists under `src/nwbforge/app/desktop.py`, the desktop shell can open manifest-backed sessions from `File -> Open Session...`, clear them with `New Session`, reopen them with `Reopen Last Session`, persist session history through `Open Recent`, default new output paths from the last used output directory, and the temporary `scripts/run_app.py` launcher now prefers the last-opened manifest when no explicit path is provided. The near-term milestone is now a first-pass internally testable desktop product, which is gated more by UI coherence, custom/hybrid workflow coverage, and operational readiness than by broad format coverage or release packaging. Broader supported-format coverage, richer desktop screens, and app-level log-retention policy are still ahead.
...
conda run -n nwbforge-dev python scripts/run_app.py --manifest path\\to\\session_manifest.json
```

The temporary launcher is a development aid only. It now boots the real desktop service composition and loads either a user-provided manifest or a generated demo manifest, without changing the long-term packaging plan.
...
1. Keep restructuring the desktop UI so it behaves like a finished product rather than a collection of functional panels.
2. Implement the first real end-to-end custom-path workflow in the desktop app.
3. Implement the first real end-to-end hybrid-path workflow in the desktop app.
4. Harden operational concerns such as preview persistence, review history, recovery flows, and report visibility for internal testing.
