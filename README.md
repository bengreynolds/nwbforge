...
3. Expand the new direct-ingest `New Session` workflow from the current heuristic-grouping, manual regrouping, simple sidecar association, role-assignment, and session-wide override baseline into richer dataset grouping and source-specific metadata handling.
4. Harden operational concerns such as richer review history, multi-snapshot recovery flows, broader desktop interaction logging beyond the current shell actions, and report visibility on top of the current latest-state persistence baseline.
