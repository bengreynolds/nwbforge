...
3. Expand the new direct-ingest `New Session` workflow from the current heuristic-grouping, role-assignment, and session-wide override baseline into richer grouping correction, sidecar association, and source-specific metadata handling.
4. Harden operational concerns such as richer review history, multi-snapshot recovery flows, broader desktop interaction logging, and report visibility on top of the current latest-state persistence baseline.
