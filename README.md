...
The current application state is still early, but it is no longer just a backend scaffold. The supported route set now includes real NeuroConv-backed CSV, Excel, image, audio, FicTrac, and DeepLabCut conversions; a shared NeuroConv adapter framework exists for additional single-interface routes; supported-route packaging now uses category-first modules for both `supported/behavior/` and `supported/tabular/`; a dedicated workflow base exists for future combined NeuroConv pipelines; and the repo now also includes the first real repo-owned custom-path workflow through `custom_session.json`, which feeds non-canonical lab metadata into the existing normalization, mapping, review, and direct PyNWB assembly path. Runtime contracts now exist for background execution, structured logging, and stage/progress/error reporting; developer setup now has a route-based package planning layer with `minimal`, `selected`, and `full` install modes; future setup/package-install UI work has backend `PackageManagementService`, `PackageInstallationService`, `ThreadedPackageInstallationExecutor`, and `PackageManagementController` boundaries to call; the repo includes toolkit-agnostic UI models under `src/nwbforge/ui/`; and it now also includes a concrete PySide6 widget baseline under `src/nwbforge/ui/qt/` for the shell, settings dialog, package-install dialog, and a conversion workspace with sectioned summary/execution/review/artifact areas, richer source details, run-overview metrics, review-guidance summaries, validation/review controls, generated-artifact visibility, direct artifact-open actions, dedicated validation/review report shortcuts, a real `Choose Output...` save dialog, docked log viewer, shell-level warning dialogs, opt-in composite file-plus-viewer logging, and a tabbed workspace for run overview, review work, and artifacts. A real desktop bootstrap/composition module now exists under `src/nwbforge/app/desktop.py`, the desktop shell can open both `session_manifest.json` and `custom_session.json` sessions from `File -> Open Session...`, clear them with `New Session`, reopen them with `Reopen Last Session`, persist session history through `Open Recent`, default new output paths from the last used output directory, and the temporary `scripts/run_app.py` launcher now prefers the last-opened session when no explicit path is provided. The near-term milestone is now a first-pass internally testable desktop product, which is gated more by UI coherence, custom/hybrid workflow coverage, and operational readiness than by broad format coverage or release packaging. Broader supported-format coverage, the first hybrid workflow, and app-level log-retention policy are still ahead.
...
2. Implement the first real end-to-end hybrid-path workflow in the desktop app.
3. Harden operational concerns such as preview persistence, review history, recovery flows, and report visibility for internal testing.
4. Expand the desktop information architecture further without sacrificing the current summary/review clarity.
